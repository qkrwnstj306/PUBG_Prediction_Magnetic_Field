...
Sanhok_map_image_path = "previous_sanhok.png" #"sanhok_img.png"
...
def generate_useful_area(phase, phase_heatmap, map_image_path, save_path="useful_areas/", threshold_factor=0.5):
    ...
    # 등고선 레벨 및 색상 (명확한 대비 적용)
    contour_levels = [50, 100, 150, 200]  
    contour_colors = [(255, 255, 255), (0, 255, 255), (0, 165, 255), (0, 0, 255)]  # 흰색 → 밝은 색 → 어두운 색

    contour_list = []
    for level in contour_levels:
        _, level_mask = cv2.threshold(norm_heatmap_uint8, level, 255, cv2.THRESH_BINARY)
        level_contours, _ = cv2.findContours(level_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        contour_list.append((level_contours, contour_colors[contour_levels.index(level)]))
    
    # 결과 이미지 생성
    output_image = map_image.copy()
    
    # 유용한 영역 외곽선 강조 (검은색 테두리 추가)
    cv2.drawContours(output_image, contours, -1, (0, 0, 0), 4, lineType=cv2.LINE_AA)  
    cv2.drawContours(output_image, contours, -1, (0, 255, 0), 3, lineType=cv2.LINE_AA)  

    # 등고선 오버레이 (외곽선 두께 조절)
    thickness_list = [1, 1, 2, 2]  # 내부일수록 얇게
    for i, (level_contours, color) in enumerate(contour_list):
        cv2.drawContours(output_image, level_contours, -1, color, thickness_list[i], lineType=cv2.LINE_AA)

    # 결과 저장
    output_image_path = os.path.join(save_path, f"sanhok_phase_{phase}_useful_overlay.png")
    cv2.imwrite(output_image_path, output_image)
    print(f"Phase {phase} 유용한 영역 및 등고선 오버레이 완료 (임계값: {thresh_value}).")

# 유용한 영역 및 등고선 오버레이 실행
for phase, phase_heatmap in aggregated_heatmaps.items():
    if phase == 0:
        continue
